// הרצה מקומית מלאה בלי התקנת Postgres:
...
const PORT_DB = 5436;
const DB_URL = `postgres://postgres:postgres@127.0.0.1:${PORT_DB}/therapy_crm`;
const SERVER_PORT = process.env.PORT || '3720';

(async () => {
  const fresh = !fs.existsSync(DATA);
  const pg = new EmbeddedPostgres({ databaseDir: DATA, user: 'postgres', password: 'postgres', port: PORT_DB, persistent: true });
  if (fresh) { console.log('▶ מאתחל Postgres נייד (פעם ראשונה)...'); await pg.initialise(); }
  console.log('▶ מפעיל Postgres...');
  await pg.start();
  try { await pg.createDatabase('therapy_crm'); console.log('✔ מסד therapy_crm נוצר'); }
  catch (e) { console.log('• המסד כבר קיים'); }

  const childEnv = { ...process.env, DATABASE_URL: DB_URL, NODE_ENV: 'development',
