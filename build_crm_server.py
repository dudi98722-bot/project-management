...
function bootstrapData(){
  ...
  // allow403: לתפקיד צופה אין הרשאה לתשלומים, ושם ריק הוא התשובה הנכונה.
  // כל שאר השגיאות חייבות להתפוצץ ולא להיבלע כרשימה ריקה.
  function getArr(path, allow403){
    return window.apiFetch(path).then(function(r){
      if(r.status===403 && allow403) return [];
  ...
  Promise.all([ getArr('/contacts/all'), getArr('/vows/all'), getArr('/payments/all', true) ]).then(function(res){
